...
We'd like to run UI tests on PR pushes. But we don't need the expensive or complexity of full ephemeral environments in Azure.

This ADR does *not* preclude also running tests against an Azure deployment for other kinds of changes. (E.g.; we'll almost certainly
run tests against Azure after a merge to `main`)
...
Chosen option: "Running the tests directly on the runner" - because it's relatively simple; and cheap to do.

### Consequences

* Good, because simpler to implement and manage
* Good, because cheaper running cost
