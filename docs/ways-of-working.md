...
* Architecture: Adhere to the [Application Architecture](./application-architecture.md). Maintain a strict separation between the stateless `RulesEngine` and the stateful `Web` application.
...
3. Once approved and CI passes, the author is responsible for merging (Squash and Merge is preferred for feature branches).
