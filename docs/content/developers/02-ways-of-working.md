...
### MVC Conventions
* Explicit routes in [WebApplicationExtensions.cs](/src/AccessingChildcareEntitlementChecker.Web/WebApplicationExtensions.cs) - as opposed to attribute routing. The rationale is that having the routes in a single journey makes them easier to update while visualising the journey as a whole.
