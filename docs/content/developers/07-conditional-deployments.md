...
1. [Azure Front Door Custom Domains](#azure-front-door-custom-domains)
2. [Azure Front Door Premium Features](#azure-front-door-premium-features)
3. [Staging Deployment Slots](#staging-deployment-slots)
...
### Behavior
While the Premium SKU is specified via `azure_frontdoor_sku = "Premium"`, the individual enterprise-grade features are enabled or disabled via separate feature flags using dynamic blocks:

* Private Link Integration: Inside `azurerm_cdn_frontdoor_origin.frontdoor-web-origin`, a dynamic `private_link` block is applied when `fd_use_private_link` is `true`. This configures Front Door to route traffic to the backend App Service using Azure's private backbone network via Private Link, rather than routing over the public internet.
* Managed WAF Rule Sets: Within the Web Application Firewall (`azurerm_cdn_frontdoor_firewall_policy.web_firewall_policy`), two comprehensive managed rulesets are applied in `Prevention` mode when `waf_enable_managed_rules` is `true`:
...
* Staging Slot Provisioning: Provisions an `azurerm_linux_web_app_slot` named `staging` attached to the main App Service when `webapp_enable_staging_slot` is `true`.
...
### Behavior
To prevent unauthorised public discovery and access during development and testing phases, a basic HTTP authentication mechanism is optionally supported:
