...
---
This reference defines the Security Architecture for the Accessing Childcare Entitlement Checker (ACEC). This reference establishes the defensive posture, identifies key actors, models threats, analyses potential attack vectors, and defines the information security profile.

As a public-facing, anonymous tool with no database and no persistent user accounts, the primary security goals of the service are:
...
This document categorises actors and components into distinct functional roles:
...
* Security Scope: Immutable library packaged with the Web Application. No external networking code or database hooks are present, preventing lateral movement or external command injection at this layer.
...
* Authentication: Microsoft Entra ID utilising Multi-Factor Authentication (MFA), Single Sign-On (SSO), and Just-In-Time (JIT) elevation via Privileged Identity Management (PIM).
...
* Security Scope: Azure Activity Logs and Log Analytics Workspace strictly audit operations.

## Anti-personas

Active threats, motives, and target assets are identified below to inform threat modelling:
...
Security assessments utilise the STRIDE methodology to systematically evaluate security boundaries across the architecture. Without a backend database, the primary threat boundaries are User to Web App and CI/CD to Cloud Infrastructure.
...
Defensive controls are aligned against major attack vectors based on the application design:
...
* Mechanism: Injecting malicious JavaScript payloads into input fields (such as child names in free-text boxes, if utilised) to execute scripts in other users' browsers or leak session cookies.
* Defensive Controls:
...
* Mechanism: Exploiting vulnerabilities in third-party NuGet packages or frontend NPM dependencies used to compile and build the site.
* Defensive Controls:
...
* In-Transit Encryption: 
  * The system globally enforces HTTPS with a minimum version of TLS 1.2.
  * The system automatically redirects all HTTP requests to HTTPS.
  * The system issues HSTS (HTTP Strict Transport Security) headers to browsers to prevent downgrade attacks.
...
* Automated DAST Scanning: The pipeline runs OWASP ZAP (Zed Attack Proxy) to perform Dynamic Application Security Testing (DAST) on non-production deployments. This test verifies that the live application has no high-risk vulnerabilities.
