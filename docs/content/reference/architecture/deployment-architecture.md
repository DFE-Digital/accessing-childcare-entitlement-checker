...
---
The cloud architecture, networking, and deployment strategy for the Accessing Childcare Entitlement Checker (ACEC) are specified in this reference. The ACEC application is a stateless ASP.NET Core web application hosted on Azure, designed for high availability, security, and scalability within the UK South region.
...
The table below indicates the composite availability of the services:

|         Scenario         | Availability |
|:------------------------:|:------------:|
|       All Services       |    99.93%    |

Approximate expected downtime:
...
* App Service Restrictions: The Web App is configured with IP restrictions to only accept traffic from the `AzureFrontDoor.Backend` service tag. This prevents users from bypassing the WAF.

## Availability & scaling

* Region: All resources are pinned to `UK South`.
* Redundancy: 
  * The Web App is configured to run with a minimum of 2 instances for high availability.
  * The deployment strategy utilizes Run-From-Package backed by an RA-GRS (Read-Access Geo-Redundant Storage) account to guarantee deployment artifact resilience.

## Deployment strategy

A Trunk-Based Development model with Release Branches is followed for higher environments.
