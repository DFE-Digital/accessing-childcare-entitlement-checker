...
This represents the highest level of abstraction, illustrating the interactions between users and the system.
...
The diagram illustrates the primary high-level technology building blocks.
...
The diagram details the internal structure of the Rules Engine and the execution sequence of eligibility processing.
...
The solution is structured into two primary functional projects:

1.  **Web**: Utilizes standard ASP.NET Core MVC patterns to manage the stateful user journey across multiple pages.
2.  **RulesEngine**: A pure logic library with no web-specific dependencies, employing a fact-based approach where raw user inputs are mapped to a `DerivedContext` prior to evaluation by independent `ISchemeEvaluator` implementations.
