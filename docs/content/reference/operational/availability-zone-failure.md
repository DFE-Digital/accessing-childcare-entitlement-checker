...
- Distributed Session Design: We store multi-step journey progress in a distributed Redis cache. An encrypted session cookie identifies the session. This design removes any server-side sticky session requirements or persistent database dependencies.
...
Because the web application is stateless and session data is stored in the distributed Redis cache, redirection of users to the remaining active instances occurs with zero data loss or session expiration, enabling seamless journey resumption.
