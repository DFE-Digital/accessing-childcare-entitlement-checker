...
The service categorizes actors and components into the following distinct functional roles:
...
To inform our threat modeling, we identify the following active threats and their corresponding motives:

```
+---------------------+---------------------------------------------------+
| Anti-persona        | Objective / Motive                                |
+---------------------+---------------------------------------------------+
| The Disruptor       | Exhaust application resources, causing service    |
| (DoS / DDoS)        | downtime to damage DfE reputation.                |
+---------------------+---------------------------------------------------+
| The Insincere User  | Manipulate input boundaries to elicit erroneous   |
| (Exploiter)         | eligibility results, potentially for political    |
|                     | leverage or to test systemic loopholes.           |
+---------------------+---------------------------------------------------+
| The Malicious       | Compromise the GitHub repository, dependencies,   |
| Supplier/Insider    | or developer machines to inject backdoors or      |
|                     | malicious redirect paths.                         |
+---------------------+---------------------------------------------------+
| The Opportunistic   | Run automated scanners to identify standard CVEs, |
| Scanner             | outdated packages, or exposed endpoints.          |
+---------------------+---------------------------------------------------+
```
...
We utilize the **STRIDE** methodology to systematically evaluate security boundaries across our architecture. Due to the lack of a backend database, our main threat boundaries are **User to Web App** and **CI/CD to Cloud Infrastructure**.

```mermaid
flowchart TD
    User([Public User]) -- HTTPS (TLS 1.2+) --> FD[Azure Front Door WAF]
    FD -- Private Link / Service Tag --> App[Azure App Service Web App]
    subgraph Trust Boundary (VNet)
...
## 5. Information Security Profile

The Information Security Profile defines how the service categorizes, protects, and governs data.

### Data Classification: OFFICIAL
Under the UK Government Security Classifications, all data handled, processed, or generated by the Childcare Entitlement Checker is classified as **OFFICIAL**.
* There is no requirement for a higher classification (Secret or Top Secret) because the tool calculates general eligibility without accessing statutory tax records, welfare systems, or credit ratings in real-time.
...
* **Data Protection APIs:**
  * For local transient session cookies, ASP.NET Core Data Protection is configured to automatically handle encryption keys, ensuring session payloads are tamper-proof and private.
