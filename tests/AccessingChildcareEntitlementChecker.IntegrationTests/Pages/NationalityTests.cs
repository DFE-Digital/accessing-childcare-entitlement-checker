...
public class NationalityTests(IntegrationTestFixture factory) : IClassFixture<IntegrationTestFixture>
{
    ...
    public async Task Get_Nationality_Has_Radios_And_BackLink()
    {
        ...
        var res = await client.GetAsync("/nationality", TestContext.Current.CancellationToken);
        ...
        Assert.NotNull(back);
        Assert.Contains("/age/parent-age", back.GetAttribute("href") ?? string.Empty);
    }
    ...
    {
        using var client = factory.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
        var get = await client.GetAsync("/nationality", TestContext.Current.CancellationToken);
        ...
        var req = new HttpRequestMessage(HttpMethod.Post, "/nationality");
        ...
        Assert.Contains("/nationality/settled-status", post.Headers.Location?.ToString() ?? string.Empty);
    }
}
